...
class JDPhone(object):
    def __init__(self, max_p):
        ...
        self.title_list = ['id', 'shopname', 'is_jd', 'price' ,'good_lv', 'mid_lv', 'bad_lv']
        # , 'brand', 'year', 'month', 'weight', 'thick', 'long',
        # 'cpu_brand', 'cpu_num',
        # 'sim_num', 'sim', 'rom', 'ram', 'size', 'front_c', 'back_c', 'battery',
        # 'total_com', 'good_com', 'mid_com', 'bad_com',
        ...
        self.driver.find_element(By.XPATH, '//*[@id="key"]').send_keys(self.key)
        self.driver.find_element(By.XPATH, "//button/i[@class='iconfont']").click()

    def set_title(self):
        '''给表格添加标题'''
        # , "品牌", "上市年份", "上市月份", "机身重量", "机身厚度", "机身长度", "CPU品牌", "CUP核数",
        #                   "单双卡类型", "SIM卡类型", "ROM", "RAM", "屏幕尺寸", "前置像素", "后置像素", "电池容量",
        #                   "总评论数", "好评数", "中评数", "差评数",
        titles = ["商品ID", "店铺名称", "是否自营", "价格", "好评率", "中评率", "差评率"]
        i = 0
    ...
    def getall(self):
        ...
        if next_page and self.max_p - 1:
            self.max_p -= 1
            self.driver.find_element(By.XPATH,"//a[@class='pn-next']").click()
            time.sleep(0.1)
